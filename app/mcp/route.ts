...
const INSTRUCTIONS = [
  ...
  "Order of calls: (1) get_account. If identity_verification.verified is false, send the user to verification_url and wait. If cards is empty, call get_payment_setup_link and wait; never ask for a card number, CVC, PIN, bank password or vault credential in chat. (2) find_products with the store URL the user gave you: it returns the exact merchant id, category slugs, currency, prices in cents and product ids. Never guess any of these from a page, name, SKU or URL. (3) create_mandate with merchant_urls, the exact categories and per_purchase_cents comfortably above the price, never equal to it: shipping, handling and tax are charged on top, so always leave headroom (find_products' mandate_hint already includes it). Then send the user authorization_url and wait. (4) get_mandate until status is active. A draft is normal; do not create a second mandate for the same request. (5) check_purchase, then purchase with products[].product_id verbatim.",
