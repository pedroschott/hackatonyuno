...
| **`find_products`** | `merchant_url`, `query?`, `category?`, `max_price_cents?`, `limit?` | Exact merchant id, category slugs, currency, product ids and prices in cents, plus a `mandate_hint` ready for `create_mandate`, whose `per_purchase_cents` already sits above the product price to absorb shipping, handling and tax |
